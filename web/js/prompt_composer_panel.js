        ...
        onclick: async () => {
          ...
          try {
            ...
            if (isTemplate && report.template_slug) {
              // opening the template runs the missing-LoRA check + download offer
              await selectTemplate(report.template_slug);
              switchTab("compose");
            }
          } catch (err) {
            errorLine.textContent = err.message;
    ...
    setEditor(
      ...
      loraCount
        ? el(
            "div",
            { class: "mrln-note" },
            `${loraCount} LoRA file(s) referenced — after import, opening the template `
              + "offers to download any that are missing (via their Civitai AIR)."
          )
        : null,
      el(
        "label",
